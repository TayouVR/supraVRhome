...
public class VRCContentManagerWindow : EditorWindow
{
    ...
    const int WORLD_IMAGE_BUTTON_HEIGHT = 80; //( 4 / 5 ) * WORLD_IMAGE_BUTTON_WIDTH; //4:5 aspect ratio, relative to width
        ...
        if (APIUser.IsLoggedInWithCredentials && uploadedWorlds != null && uploadedAvatars != null)
        {
            ...
            if (uploadedWorlds.Count > 0)
            {
                ...
                foreach (ApiWorld w in tmpWorlds)
                {
                    ...
                    {
                        if (GUILayout.Button(ImageCache[w.id], GUILayout.Height(WORLD_IMAGE_BUTTON_HEIGHT), GUILayout.Width(WORLD_IMAGE_BUTTON_WIDTH)))
                        {
                    ...
                    {
                        if (GUILayout.Button("", GUILayout.Height(WORLD_IMAGE_BUTTON_HEIGHT), GUILayout.Width(WORLD_IMAGE_BUTTON_WIDTH)))
                        {
                            Application.OpenURL(w.imageUrl);
                        }
                    }
            ...
            if (uploadedAvatars.Count > 0)
            {
                ...
                foreach (ApiAvatar a in tmpAvatars)
                {
                    ...
                    {
                        if (GUILayout.Button(ImageCache[a.id], GUILayout.Height(WORLD_IMAGE_BUTTON_HEIGHT), GUILayout.Width(AVATAR_IMAGE_BUTTON_WIDTH)))
                        {
                    ...
                    {
                        if (GUILayout.Button("", GUILayout.Height(WORLD_IMAGE_BUTTON_HEIGHT), GUILayout.Width(AVATAR_IMAGE_BUTTON_WIDTH)))
                        {
                            Application.OpenURL(a.imageUrl);
                        }
                    }
